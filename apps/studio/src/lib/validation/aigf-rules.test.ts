...
describe('aigf-001: AI node missing AIGF controls', () => {
  it('warns when AI node has no aigf-prefixed controls', () => {
  ...
  it('does NOT warn when AI node has aigf-prefixed controls', () => {
    const controls: CalmControls = {
      'aigf-version-pinning': { description: 'Pin LLM version', requirements: [] },
    };
...
describe('aigf-002: ai:llm missing version pinning', () => {
  it('warns when ai:llm node lacks aigf-version-pinning control', () => {
    const arch = makeArch([makeAINode('ai:llm')]);
    const issues = runAIGFRules(arch);
  ...
  it('does NOT warn when ai:llm has aigf-version-pinning', () => {
    const controls: CalmControls = {
      'aigf-version-pinning': { description: 'Pinned to gpt-4-0613', requirements: [] },
    };
    const arch = makeArch([makeAINode('ai:llm', controls)]);
    const issues = runAIGFRules(arch);
    const rule = issues.filter(i => i.message.includes('aigf-002'));
...
describe('aigf-003: ai:vector-store missing data classification', () => {
  it('warns when ai:vector-store lacks aigf-data-classification', () => {
...
describe('aigf-004: ai:agent missing least privilege', () => {
  it('returns error when ai:agent lacks aigf-least-privilege', () => {
...
describe('aigf-005: MCP connection without aigf-mcp-security', () => {
  it('returns error when relationship with MCP in description but source node missing aigf-mcp-security', () => {
...
describe('aigf-006: multi-agent without isolation', () => {
  it('warns when >1 ai:agent and none has aigf-isolation', () => {
    const arch = makeArch([
...
describe('aigf-007: ai:rag-pipeline missing citation traceability', () => {
  it('returns info when ai:rag-pipeline lacks aigf-citation-traceability', () => {
...
describe('aigf-008: ai:vector-store missing encryption at rest', () => {
  it('warns when ai:vector-store lacks aigf-encryption-at-rest', () => {
    const arch = makeArch([makeAINode('ai:vector-store')]);
    const issues = runAIGFRules(arch);
...
describe('aigf-009: ai:agent connected to ai:tool without tool chain validation', () => {
  it('warns when ai:agent connects to ai:tool without aigf-tool-chain-validation', () => {
